...
func handleAuthExchange(r *fastglue.Request) error {
	...
	if len(claims.PhoneNumberCountryCode) > maxPhoneCountryCodeLength {
		return r.SendErrorEnvelope(fasthttp.StatusBadRequest, app.i18n.Ts("globals.messages.maxLength", "max", strconv.Itoa(maxPhoneCountryCodeLength)), nil, envelope.InputError)
	}
